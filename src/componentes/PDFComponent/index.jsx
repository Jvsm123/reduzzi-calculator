...
Font.register({ family: "axiforma", src: "https://fonts.gstatic.com/s/axiforma/v10/1Ptsg8LJRfWJmhDAuUsw0L5OcCk.ttf" });
...
const ContainerComparativo = ({ header, content, container }) => (
  ...
  >
    <div style={tw("flex flex-row items-start gap-2 w-[100%] mb-6")}>
      ...
      <Text style={tw("text-[12px] font-extrabold text-[#00395e] max-w-[140px]")}>
        {header}
      </Text>
    </div>
    ...
    <PDFViewer style={{ width: window.innerWidth, height: window.innerHeight }}>
      <Document>
        <Page size="A4">
          ...
          <View style={tw("bg-white h-[720px] flex flex-col items-center gap-8 text-white")}>
            <div style={tw("flex flex-row justify-center items-center mt-4 gap-2")}>
              <Image src={arrowGray} style={tw("w-[10px] h-[10px]")} />
            ...
            <div style={tw("flex flex-row text-sm text-gray-600 gap-[8rem]")}>
              <ValoresGerais
                ...
                divStyle={tw("px-2 border-[1px] border-gray-400 rounded-lg w-[90px] text-base items-center text-start pt-2 w-[150px]")}
              />
              <ValoresGerais
                label="Na regra atual"
                value={`${Number(data?.totalImpostoComReducao).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}`}
                textStyle={tw("text-gray-600 font-extrabold text-[16px] text-[#036838ff]")}
                valueStyle={tw("text-gray-500 font-bold text-[20px] text-[#093957ff]")}
                divStyle={tw("px-2 border-[1px] border-gray-400 rounded-lg w-[90px] text-base items-center text-start pt-2 w-[150px]")}
              />
            </div>

            <div style={tw("bg-[#006837ff] flex flex-row justify-between rounded-lg md:overflow-hidden flex-wrap lg:flex-nowrap")}>
              <div style={tw("flex md:items-center justify-between w-full lg:w-[340px] py-6 px-8 flex-col md:flex-row gap-7")}>
                <div>
                  <Text style={tw("border-l-[5px] border-green-400 font-axiforma text-white font-extrabold text-[14px] pl-3")}>
                    ECONOMIA
                  </Text>
                  <Text style={tw("border-l-[5px] border-green-400 text-white font-extrabold text-[14px] pl-3")}>GERADA:</Text>
                </div>
                <Text style={tw("text-white font-extrabold text-[20px] sm:text-[30px]")}>
                {`${Number(data?.totalImpostoSemReducao - data?.totalImpostoComReducao).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}`}</Text>
              </div>

              <div style={tw("bg-[#063958ff] text-center py-3 px-10 w-full lg:max-w-[250px]")}>
                <Text style={tw("text-white font-extrabold text-[20px]")}>
                  {`${(
                    100 -
                    (Number(data?.totalImpostoComReducao) / data?.totalImpostoSemReducao) *
                      100
                ...
                <Text style={tw("text-white font-bold text-[8px] text-center")}>CONFORME IN. 2021/2021</Text>
              </div>
            </div>
            ...
            <div style={tw("flex flex-row gap-6")}>
                ...
                content={[
                    ...
                    <div style={tw("flex flex-col gap-3 items-center pl-1")}>
                      ...
                      <div style={tw("flex flex-row items-center gap-2")}>
                        <div style={tw("flex flex-row items-center")}>
                           <Text style={tw("text-[#063958ff] text-[12px]")}>{data?.valorFinalDaObraParcelamento}</Text>
                          <Text style={tw("text-[10px]")}>x</Text>
                        </div>
                        <Text style={tw("text-[#063958ff] text-[12px]")}>{Number(data?.valorDoParcelamentoTotal).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</Text>
                        <Text style={tw("text-[6px]")}>NO DÉBITO AUTOMÁTICO</Text>
                      </div>
                    </div>
                  </div>,
                  ...
                  <div style={tw("flex flex-row items-center gap-10")}>
                    ...
                    <div style={tw("flex flex-row gap-3 flex-end")}>
                      ...
                      <Text style={tw("text-[#063958ff]")}>{Number(data?.totalImpostoComReducao).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</Text>
                    </div>
                  </div>,
                ]}
              />
              <div style={tw("flex flex-col justify-between ")}>
                <ContainerComparativo
                  ...
                  container={'w-[210px] h-[10rem] flex flex-col items-center bg-white border-[1px] border-gray-700 rounded-lg p-6'}
                  content={[
                    ...
                    <>
                      ...
                      <div style={tw("flex flex-row")}>
                        ...
                        <Text>de</Text>
                        <Text>{typeof data.honorarioValor === 'string' && data.honorarioValor || (Number(data?.honorarioValor) / 10).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</Text>
                      </div>
                    }
                    </>,
                  ]}
                ...
                <Text style={tw("text-gray-500 w-[16rem] text-[10px] mb-[2rem]")}>
                    *Este orçamento tem cunho informativo, com valores aproximados, ao fazer a analise
                    detalhada da documentação da obra os valores
                    podem ser alterados de acordo com as datas e
                    metragem do imóvel
                </Text>
              </div>
            </div>

            <div style={tw("bg-[#00395e] flex flex-row h-[12rem] w-[100%] justify-between items-center px-14")}>
              <div style={tw("flex flex-col text-[8px] w-[260px]")}>
                ...
                <div style={tw("flex flex-col gap-4")}>
                  <div style={tw("flex flex-row items-center gap-2")}>
                    ...
                    <Text style={tw("text-white")}>SEGURANÇA JURÍDICA E TRIBUTÁRIA</Text>
                  </div>

                  <div style={tw("flex flex-row items-center gap-2")}>
                    <Image src={correct} style={tw("w-[20px] h-[20px]")}/>
                    <Text style={tw("text-white")}>PARCELAMENTO E FLEXIBILIDADE DE PAGAMENTO</Text>
                  </div>

                  <div style={tw("flex flex-row items-center gap-2")}>
                    <Image src={correct} style={tw("w-[21px] h-[20px]")}/>
                    <Text style={tw("text-white")}>RESPONSABILIDADE POR ATÉ 5 ANOS NOS VALORES PASSADOS</Text>
                  </div>
                </div>
              </div>
